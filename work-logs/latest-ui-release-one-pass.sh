...
if [[ ! -t 0 ]]; then
  if [[ "${MC_AGENT_MODE:-}" == "true" ]]; then
    echo '[ui-release-one-pass] INFO: Bypassing interactive terminal check in MC_AGENT_MODE.'
  else
...
EXPECTED_WEB_DIR_PATH=/mnt/d/Warung\ Kerja\ 1.0/03_Active_Projects/Mission\ Control/mission-control-v2/apps/web
CURRENT_WORKING_DIR=$(pwd -P)
if [[ "$CURRENT_WORKING_DIR" != "$EXPECTED_WEB_DIR_PATH" ]]; then
  echo '[ui-release-one-pass] ERROR: Working directory path mismatch before release gate prerequisites.'
  echo "[ui-release-one-pass] expectedWorkingDir=$EXPECTED_WEB_DIR_PATH currentWorkingDir=$CURRENT_WORKING_DIR"
  echo '[ui-release-one-pass] Re-run the prepared one-pass script without changing directories after launch.'
  exit 1
fi
echo '[ui-release-one-pass] Verified working directory path matches generated web directory before release gate prerequisites.'
...
EXPECTED_WEB_DIR=$(pwd -P)
if [[ "$EXPECTED_WEB_DIR" != "$EXPECTED_WEB_DIR_PATH" ]]; then
  echo '[ui-release-one-pass] ERROR: Current working directory drifted from generated web directory before release gate.'
  echo "[ui-release-one-pass] expectedWorkingDir=$EXPECTED_WEB_DIR_PATH currentWorkingDir=$EXPECTED_WEB_DIR"
  echo '[ui-release-one-pass] Re-run the prepared one-pass script from the generated web directory path.'
  exit 1
fi
echo '[ui-release-one-pass] Verified current working directory still matches generated web directory before release gate.'
CURRENT_GIT_TOPLEVEL=$(git rev-parse --show-toplevel 2>/dev/null || echo "unknown")
if [[ "$CURRENT_GIT_TOPLEVEL" == "unknown" ]]; then
  echo '[ui-release-one-pass] ERROR: Not inside a valid git repository before release gate.'
  echo '[ui-release-one-pass] Ensure script runs from the intended web repository work-tree, then retry.'
  exit 1
fi
echo '[ui-release-one-pass] Verified git repository context exists before release gate.'
GIT_PREFIX=$(git rev-parse --show-prefix 2>/dev/null || echo "unknown")
if [[ "$GIT_PREFIX" == "unknown" ]]; then
  echo '[ui-release-one-pass] ERROR: Could not determine git prefix before release gate.'
  echo '[ui-release-one-pass] Ensure repository metadata is intact, then retry.'
  exit 1
fi
echo '[ui-release-one-pass] Verified git execution context prefix before release gate.'
if [[ "$(git rev-parse --is-inside-work-tree 2>/dev/null || echo "false")" != "true" ]]; then
  echo '[ui-release-one-pass] ERROR: Current directory is not an active git work tree before release gate.'
  echo '[ui-release-one-pass] Ensure repository metadata is intact and rerun from the web repository root.'
  exit 1
fi
echo '[ui-release-one-pass] Verified active git work-tree context before release gate.'
if [[ "$(git rev-parse --is-inside-git-dir 2>/dev/null || echo "false")" == "true" ]]; then
  echo '[ui-release-one-pass] ERROR: Git execution context is inside .git directory before release gate.'
  echo '[ui-release-one-pass] Run from the repository work-tree root (not inside .git metadata paths), then retry.'
  exit 1
fi
echo '[ui-release-one-pass] Verified git execution context is outside .git metadata directory before release gate.'
GIT_COMMON_DIR=$(git rev-parse --git-common-dir 2>/dev/null || echo "unknown")
if [[ "$GIT_COMMON_DIR" == "unknown" || ! -d "$GIT_COMMON_DIR" ]]; then
  echo '[ui-release-one-pass] ERROR: Git common-dir is missing or invalid before release gate.'
  echo "[ui-release-one-pass] gitCommonDir=$GIT_COMMON_DIR"
  echo '[ui-release-one-pass] Ensure repository git metadata is intact, then rerun from the web repository root.'
  exit 1
fi
RESOLVED_GIT_COMMON_DIR=$(cd "$GIT_COMMON_DIR" && pwd -P)
echo '[ui-release-one-pass] Verified git common-dir exists before release gate.'
GIT_DIR=$(git rev-parse --git-dir 2>/dev/null || echo "unknown")
if [[ "$GIT_DIR" == "unknown" || ! -d "$GIT_DIR" ]]; then
  echo '[ui-release-one-pass] ERROR: Git dir is missing or invalid before release gate.'
  echo "[ui-release-one-pass] gitDir=$GIT_DIR"
  echo '[ui-release-one-pass] Ensure repository git dir metadata is intact, then rerun from the web repository root.'
  exit 1
fi
RESOLVED_GIT_DIR=$(cd "$GIT_DIR" && pwd -P)
echo '[ui-release-one-pass] Verified git dir exists before release gate.'
...
UNTRACKED_PRE_GATE_FILES=$(git ls-files --others --exclude-standard)
if [[ -n "$UNTRACKED_PRE_GATE_FILES" ]]; then
  echo '[ui-release-one-pass] ERROR: Untracked git files detected before release gate.'
  echo '[ui-release-one-pass] Commit, stash, or clean untracked files, then rerun.'
  printf "%s\n" "$UNTRACKED_PRE_GATE_FILES"
  exit 1
fi
echo '[ui-release-one-pass] Git untracked files clean before release gate.'
if [[ -f .git/MERGE_HEAD || -d .git/rebase-apply || -d .git/rebase-merge || -f .git/CHERRY_PICK_HEAD || -f .git/REVERT_HEAD || -f .git/BISECT_LOG ]]; then
  echo '[ui-release-one-pass] ERROR: Git operation in progress (merge/rebase/cherry-pick/revert/bisect) before release gate.'
  echo '[ui-release-one-pass] Complete or abort in-progress git operations, then rerun.'
  git status --short --branch
  exit 1
fi
echo '[ui-release-one-pass] Verified no in-progress git operations before release gate.'
if [[ -f .git/index.lock ]]; then
  echo '[ui-release-one-pass] ERROR: Git index lock detected before release gate (.git/index.lock).'
  echo '[ui-release-one-pass] Ensure no concurrent git process is running, remove stale lock, then rerun.'
  ls -l .git/index.lock
  exit 1
fi
echo '[ui-release-one-pass] Verified no git index lock before release gate.'
...
RELEASE_GATE_EXPECTED_BUNDLE_MANIFEST_PATH=$(node -e "const fs=require(\"fs\");const d=JSON.parse(fs.readFileSync(process.argv[1],\"utf8\"));process.stdout.write(String(d.artifacts?.latestBundleManifest?.path||\"unknown\"));" work-logs/latest-ui-release-handoff-bundle-manifest.json 2>/dev/null || echo "unknown")
...
RELEASE_GATE_EXPECTED_WEB_DIR_PATH=$(pwd -P)
export RELEASE_GATE_RUN_TOKEN RELEASE_GATE_EXPECTED_COMMIT RELEASE_GATE_EXPECTED_BRANCH RELEASE_GATE_EXPECTED_READINESS_TIMESTAMP RELEASE_GATE_EXPECTED_READINESS_STATUS RELEASE_GATE_EXPECTED_BLOCKING_CHECKS RELEASE_GATE_EXPECTED_READINESS_SHA256 RELEASE_GATE_EXPECTED_READINESS_REPORT_JSON_SHA256 RELEASE_GATE_EXPECTED_READINESS_REPORT_MD_SHA256 RELEASE_GATE_EXPECTED_BUNDLE_MANIFEST_SHA256 RELEASE_GATE_EXPECTED_BUNDLE_MANIFEST_PATH RELEASE_GATE_EXPECTED_HANDOFF_SHA256 RELEASE_GATE_EXPECTED_OPERATOR_BRIEF_SHA256 RELEASE_GATE_EXPECTED_ONE_PASS_SHA256 RELEASE_GATE_EXPECTED_WEB_DIR_PATH
for REQUIRED_GATE_METADATA_VAR in RELEASE_GATE_EXPECTED_COMMIT RELEASE_GATE_EXPECTED_BRANCH RELEASE_GATE_EXPECTED_READINESS_TIMESTAMP RELEASE_GATE_EXPECTED_READINESS_STATUS RELEASE_GATE_EXPECTED_BLOCKING_CHECKS RELEASE_GATE_EXPECTED_READINESS_SHA256 RELEASE_GATE_EXPECTED_READINESS_REPORT_JSON_SHA256 RELEASE_GATE_EXPECTED_READINESS_REPORT_MD_SHA256 RELEASE_GATE_EXPECTED_BUNDLE_MANIFEST_SHA256 RELEASE_GATE_EXPECTED_BUNDLE_MANIFEST_PATH RELEASE_GATE_EXPECTED_HANDOFF_SHA256 RELEASE_GATE_EXPECTED_OPERATOR_BRIEF_SHA256 RELEASE_GATE_EXPECTED_ONE_PASS_SHA256 RELEASE_GATE_EXPECTED_WEB_DIR_PATH; do
...
if [[ "$RELEASE_GATE_EXPECTED_BRANCH" == "HEAD" ]]; then
  echo '[ui-release-one-pass] ERROR: Detached HEAD detected for pre-gate metadata.'
  echo "[ui-release-one-pass] branch=$RELEASE_GATE_EXPECTED_BRANCH"
  echo '[ui-release-one-pass] Check out a named branch before running release gate assertions.'
  exit 1
fi
echo '[ui-release-one-pass] Verified branch is attached (not detached HEAD) for pre-gate evidence.'
CURRENT_PRE_GATE_COMMIT=$(git rev-parse --short HEAD 2>/dev/null || echo "unknown")
CURRENT_PRE_GATE_BRANCH=$(git rev-parse --abbrev-ref HEAD 2>/dev/null || echo "unknown")
CURRENT_RESOLVED_GIT_COMMON_DIR=$(cd "$GIT_COMMON_DIR" && pwd -P 2>/dev/null || echo "unknown")
CURRENT_RESOLVED_GIT_DIR=$(cd "$GIT_DIR" && pwd -P 2>/dev/null || echo "unknown")
if [[ "$CURRENT_PRE_GATE_COMMIT" != "$RELEASE_GATE_EXPECTED_COMMIT" || "$CURRENT_PRE_GATE_BRANCH" != "$RELEASE_GATE_EXPECTED_BRANCH" ]]; then
  echo '[ui-release-one-pass] ERROR: Pre-gate git HEAD metadata drift detected before release gate execution.'
  echo "[ui-release-one-pass] capturedCommit=$RELEASE_GATE_EXPECTED_COMMIT currentCommit=$CURRENT_PRE_GATE_COMMIT capturedBranch=$RELEASE_GATE_EXPECTED_BRANCH currentBranch=$CURRENT_PRE_GATE_BRANCH"
  echo '[ui-release-one-pass] Re-run release:ui-ready-handoff to recapture stable git metadata before gate execution.'
  exit 1
fi
echo '[ui-release-one-pass] Verified pre-gate git HEAD metadata remains stable before gate execution.'
if [[ "$CURRENT_RESOLVED_GIT_COMMON_DIR" != "$RESOLVED_GIT_COMMON_DIR" || "$CURRENT_RESOLVED_GIT_DIR" != "$RESOLVED_GIT_DIR" ]]; then
  echo '[ui-release-one-pass] ERROR: Pre-gate resolved git metadata path drift detected before release gate execution.'
  echo "[ui-release-one-pass] capturedGitCommonDir=$RESOLVED_GIT_COMMON_DIR currentGitCommonDir=$CURRENT_RESOLVED_GIT_COMMON_DIR capturedGitDir=$RESOLVED_GIT_DIR currentGitDir=$CURRENT_RESOLVED_GIT_DIR"
  echo '[ui-release-one-pass] Re-run release:ui-ready-handoff to recapture stable git metadata paths before gate execution.'
  exit 1
fi
echo '[ui-release-one-pass] Verified resolved git metadata paths remain stable before gate execution.'
...
CURRENT_BUNDLE_MANIFEST_PATH=$(node -e "const fs=require(\"fs\");const d=JSON.parse(fs.readFileSync(process.argv[1],\"utf8\"));process.stdout.write(String(d.artifacts?.latestBundleManifest?.path||\"unknown\"));" work-logs/latest-ui-release-handoff-bundle-manifest.json 2>/dev/null || echo "unknown")
if [[ "$CURRENT_BUNDLE_MANIFEST_PATH" != "work-logs/latest-ui-release-handoff-bundle-manifest.json" ]]; then
  echo '[ui-release-one-pass] ERROR: Stable bundle manifest self-path is invalid before release gate execution.'
  echo "[ui-release-one-pass] currentManifestSelfPath=$CURRENT_BUNDLE_MANIFEST_PATH expected=work-logs/latest-ui-release-handoff-bundle-manifest.json"
  echo '[ui-release-one-pass] Re-run release:ui-ready-handoff to regenerate a stable bundle manifest self-entry before gate execution.'
  exit 1
fi
if [[ "$CURRENT_BUNDLE_MANIFEST_PATH" != "$RELEASE_GATE_EXPECTED_BUNDLE_MANIFEST_PATH" ]]; then
  echo '[ui-release-one-pass] ERROR: Pre-gate handoff bundle manifest self-path drift detected before release gate execution.'
  echo "[ui-release-one-pass] capturedManifestSelfPath=$RELEASE_GATE_EXPECTED_BUNDLE_MANIFEST_PATH currentManifestSelfPath=$CURRENT_BUNDLE_MANIFEST_PATH"
  echo '[ui-release-one-pass] Re-run release:ui-ready-handoff to recapture stable bundle manifest self-entry before gate execution.'
  exit 1
fi
echo '[ui-release-one-pass] Verified stable handoff bundle manifest self-path before release gate execution.'
CURRENT_HANDOFF_SHA256=$(sha256sum work-logs/latest-ui-release-handoff.md 2>/dev/null | awk "{print \$1}" || echo "unknown")
if [[ ! "$CURRENT_HANDOFF_SHA256" =~ ^[a-f0-9]{64}$ ]]; then
  echo '[ui-release-one-pass] ERROR: Invalid current handoff markdown SHA-256 format for pre-gate drift assertion.'
  echo "[ui-release-one-pass] currentHandoffSha256=$CURRENT_HANDOFF_SHA256"
  echo '[ui-release-one-pass] Ensure latest-ui-release-handoff.md remains present and hashable before gate execution.'
  exit 1
fi
if [[ "$CURRENT_HANDOFF_SHA256" != "$RELEASE_GATE_EXPECTED_HANDOFF_SHA256" ]]; then
  echo '[ui-release-one-pass] ERROR: Pre-gate handoff markdown SHA-256 drift detected before release gate execution.'
  echo "[ui-release-one-pass] capturedHandoffSha256=$RELEASE_GATE_EXPECTED_HANDOFF_SHA256 currentHandoffSha256=$CURRENT_HANDOFF_SHA256"
  echo '[ui-release-one-pass] Re-run release:ui-ready-handoff to recapture stable handoff markdown before gate execution.'
  exit 1
fi
echo '[ui-release-one-pass] Verified pre-gate handoff markdown SHA-256 remains stable before gate execution.'
CURRENT_OPERATOR_BRIEF_SHA256=$(sha256sum work-logs/latest-ui-release-operator-brief.md 2>/dev/null | awk "{print \$1}" || echo "unknown")
if [[ ! "$CURRENT_OPERATOR_BRIEF_SHA256" =~ ^[a-f0-9]{64}$ ]]; then
  echo '[ui-release-one-pass] ERROR: Invalid current operator brief SHA-256 format for pre-gate drift assertion.'
  echo "[ui-release-one-pass] currentOperatorBriefSha256=$CURRENT_OPERATOR_BRIEF_SHA256"
  echo '[ui-release-one-pass] Ensure latest-ui-release-operator-brief.md remains present and hashable before gate execution.'
  exit 1
fi
if [[ "$CURRENT_OPERATOR_BRIEF_SHA256" != "$RELEASE_GATE_EXPECTED_OPERATOR_BRIEF_SHA256" ]]; then
  echo '[ui-release-one-pass] ERROR: Pre-gate operator brief SHA-256 drift detected before release gate execution.'
  echo "[ui-release-one-pass] capturedOperatorBriefSha256=$RELEASE_GATE_EXPECTED_OPERATOR_BRIEF_SHA256 currentOperatorBriefSha256=$CURRENT_OPERATOR_BRIEF_SHA256"
  echo '[ui-release-one-pass] Re-run release:ui-ready-handoff to recapture stable operator brief before gate execution.'
  exit 1
fi
echo '[ui-release-one-pass] Verified pre-gate operator brief SHA-256 remains stable before gate execution.'
CURRENT_ONE_PASS_SHA256=$(sha256sum work-logs/latest-ui-release-one-pass.sh 2>/dev/null | awk "{print \$1}" || echo "unknown")
if [[ ! "$CURRENT_ONE_PASS_SHA256" =~ ^[a-f0-9]{64}$ ]]; then
  echo '[ui-release-one-pass] ERROR: Invalid current one-pass script SHA-256 format for pre-gate drift assertion.'
  echo "[ui-release-one-pass] currentOnePassSha256=$CURRENT_ONE_PASS_SHA256"
  echo '[ui-release-one-pass] Ensure latest-ui-release-one-pass.sh remains present and hashable before gate execution.'
  exit 1
fi
if [[ "$CURRENT_ONE_PASS_SHA256" != "$RELEASE_GATE_EXPECTED_ONE_PASS_SHA256" ]]; then
  echo '[ui-release-one-pass] ERROR: Pre-gate one-pass script SHA-256 drift detected before release gate execution.'
  echo "[ui-release-one-pass] capturedOnePassSha256=$RELEASE_GATE_EXPECTED_ONE_PASS_SHA256 currentOnePassSha256=$CURRENT_ONE_PASS_SHA256"
  echo '[ui-release-one-pass] Re-run release:ui-ready-handoff to recapture stable one-pass script before gate execution.'
  exit 1
fi
echo '[ui-release-one-pass] Verified pre-gate one-pass script SHA-256 remains stable before gate execution.'
...
echo "[ui-release-one-pass] Expected web directory path for gate evidence: $RELEASE_GATE_EXPECTED_WEB_DIR_PATH"
npm run release:ui-gate
if [[ ! -f /mnt/d/Warung\ Kerja\ 1.0/03_Active_Projects/Mission\ Control/mission-control-v2/work-logs/latest-ui-smoke-release-gate.md ]]; then
...
if ! grep -Fq "Expected handoff bundle manifest path from one-pass pre-gate check: \\`$RELEASE_GATE_EXPECTED_BUNDLE_MANIFEST_PATH\\`" work-logs/latest-ui-smoke-release-gate.md; then
  echo '[ui-release-one-pass] ERROR: Smoke evidence handoff bundle manifest path mismatch for this release:ui-gate run.'
  echo "[ui-release-one-pass] expectedManifestPath=$RELEASE_GATE_EXPECTED_BUNDLE_MANIFEST_PATH"
  echo '[ui-release-one-pass] Ensure release gate records expected handoff bundle manifest path metadata and retry.'
  exit 1
fi
...
if ! grep -Fq "Expected web directory path from one-pass pre-gate check: \\`$RELEASE_GATE_EXPECTED_WEB_DIR_PATH\\`" work-logs/latest-ui-smoke-release-gate.md; then
  echo '[ui-release-one-pass] ERROR: Smoke evidence web directory path mismatch for this release:ui-gate run.'
  echo "[ui-release-one-pass] expectedWebDir=$RELEASE_GATE_EXPECTED_WEB_DIR_PATH"
  echo '[ui-release-one-pass] Ensure release gate records expected web directory path metadata and retry.'
  exit 1
fi
if ! grep -q UI\ release\ gate:\ ✅\ PASS /mnt/d/Warung\ Kerja\ 1.0/03_Active_Projects/Mission\ Control/mission-control-v2/work-logs/latest-ui-smoke-release-gate.md; then
...
echo '[ui-release-one-pass] Verified handoff bundle manifest path match in smoke evidence.'
...
echo '[ui-release-one-pass] Verified web directory path match in smoke evidence.'
